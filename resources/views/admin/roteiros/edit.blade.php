...
<div class="row mb-2">
    <div class="col-sm-6">
        <h1><i class="fas fa-search mr-2"></i>Editar Roteiro</h1>
    </div>
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{route('home')}}">Painel de Controle</a></li>
            <li class="breadcrumb-item"><a href="{{route('roteiros.index')}}">Roteiros</a></li>
            <li class="breadcrumb-item active">Editar Roteiro</li>
        </ol>
    </div>
</div> 
@stop
...
<form action="{{ route('roteiros.update',['id' => $roteiro->id]) }}" method="post" enctype="multipart/form-data" autocomplete="off">
@csrf
@method('PUT')         
<div class="row">            
<div class="col-12">
    ...
    <div class="tab-pane fade show active" id="custom-tabs-four-home" role="tabpanel" aria-labelledby="custom-tabs-four-home-tab">
        ...
        <div class="row mb-2"> 
            <div class="col-12 col-md-4 col-lg-5"> 
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="name" value="{{ old('name') ?? $roteiro->name }}">
                </div>
            </div> 
            <div class="col-12 col-md-6 col-lg-5">   
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="headline" value="{{old('headline') ?? $roteiro->headline}}">
                </div>                                                    
            </div>          
            ...
            <div class="col-3 col-md-4 col-lg-2">
                <div class="form-group">
                    ...
                    <select name="status" class="form-control">
                        <option value="1" {{ (old('status') == '1' ? 'selected' : ($roteiro->status == '1' ? 'selected' : '')) }}>Publicado</option>
                        <option value="0" {{ (old('status') == '0' ? 'selected' : ($roteiro->status == '0' ? 'selected' : '')) }}>Rascunho</option>
                    </select>
                </div>
            </div>                           
        </div>
        <div class="col-12 mb-1"> 
            <div class="form-group">
                ...
                <input id="tags_1" class="tags" rows="5" name="metatags" value="{{ old('metatags') ?? $roteiro->metatags }}">
            </div>
        </div>
        <div class="row mb-2">
            <div class="col-12">   
                <label class="labelforms text-muted"><b>Descrição do Roteiro</b></label>
                <x-adminlte-text-editor name="content" v placeholder="Descrição do roteiro..." :config="$config">{{ old('content') ?? $roteiro->content }}</x-adminlte-text-editor>                                                                                     
            </div>
            <div class="col-12">   
                <label class="labelforms text-muted"><b>Informações Adicionais</b></label>
                <textarea id="inputDescription" class="form-control" rows="5" name="notasadicionais">{{ old('notasadicionais') ?? $roteiro->notasadicionais }}</textarea>                                                      
            </div>
            <div class="col-12 mt-3">   
                ...
                <textarea id="inputDescription" class="form-control" rows="5" name="mapadogoogle">{{ old('mapadogoogle') ?? $roteiro->mapadogoogle }}</textarea>                                                      
            </div>
        </div>
        ...
        <div class="row mb-4">
            <div class="col-12 col-sm-12 col-md-6 col-lg-6"> 
                <div class="form-group">
                    ...
                    <div class="form-check">
                        <input id="exibirmarcadaguasim" class="form-check-input" type="radio" value="1" name="exibirmarcadagua" {{(old('exibirmarcadagua') == '1' ? 'checked' : ($roteiro->exibirmarcadagua == '1' ? 'checked' : ''))}}>
                        <label for="exibirmarcadaguasim" class="form-check-label mr-5">Sim</label>
                        <input id="exibirmarcadaguanao" class="form-check-input" type="radio" value="0" name="exibirmarcadagua" {{(old('exibirmarcadagua') == '0' ? 'checked' : ($roteiro->exibirmarcadagua == '0' ? 'checked' : ''))}}>
            ...
            <div class="col-12 col-sm-12 col-md-6 col-lg-6">   
                <div class="form-group">
                    ...
                    <input type="text" class="form-control"  name="legendaimgcapa" value="{{ old('legendaimgcapa') ?? $roteiro->legendaimgcapa }}">
                </div>                                                    
            </div>
            <div class="col-sm-12">                                        
                <div class="form-group">
                ...
                <div class="property_image">
                    ...
                    <div class="property_image_item">
                        ...
                        <div class="property_image_actions">
                            <a href="javascript:void(0)" class="btn btn-xs {{ ($image->cover == true ? 'btn-success' : 'btn-default') }} icon-notext image-set-cover px-2" data-action="{{ route('roteiros.imageSetCover', ['image' => $image->id]) }}"><i class="nav-icon fas fa-check"></i> </a>
                            <a href="javascript:void(0)" class="btn btn-danger btn-xs image-remove px-2" data-action="{{ route('roteiros.imageRemove', ['image' => $image->id]) }}"><i class="nav-icon fas fa-times"></i> </a>
                        </div>
                    </div>
                    @endforeach
                </div>
