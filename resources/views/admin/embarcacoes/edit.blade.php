...
@section('title', 'Editar Embarcação')
...
<div class="row mb-2">
    <div class="col-sm-6">
        <h1><i class="fas fa-search mr-2"></i>Editar Embarcação</h1>
    </div>
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{route('home')}}">Painel de Controle</a></li>
            <li class="breadcrumb-item"><a href="{{route('embarcacoes.index')}}">Embarcações</a></li>
            <li class="breadcrumb-item active">Editar Embarcação</li>
        </ol>
    </div>
</div> 
@stop
...
<form action="{{ route('embarcacoes.update',['id' => $embarcacao->id]) }}" method="post" enctype="multipart/form-data" autocomplete="off">
@csrf
@method('PUT')         
<div class="row">            
<div class="col-12">
    ...
    <div class="tab-pane fade show active" id="custom-tabs-four-home" role="tabpanel" aria-labelledby="custom-tabs-four-home-tab">
        ...
        <div class="row mb-2"> 
            <div class="col-12 col-md-4 col-lg-5"> 
                <div class="form-group">
                    <label class="labelforms text-muted"><b>*Nome da Embarcação</b> </label>
                    <input type="text" class="form-control" name="name" value="{{ old('name') ?? $embarcacao->name }}">
                </div>
            </div> 
            <div class="col-12 col-md-6 col-lg-5">   
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="headline" value="{{old('headline') ?? $embarcacao->headline}}">
                </div>                                                    
            </div>          
            ...
            <div class="col-3 col-md-4 col-lg-2">
                <div class="form-group">
                    ...
                    <select name="status" class="form-control">
                        <option value="1" {{ (old('status') == '1' ? 'selected' : ($embarcacao->status == '1' ? 'selected' : '')) }}>Publicado</option>
                        <option value="0" {{ (old('status') == '0' ? 'selected' : ($embarcacao->status == '0' ? 'selected' : '')) }}>Rascunho</option>
                    </select>
                </div>
            </div>                           
        </div>
        <div class="row mb-2">
            <div class="col-12 col-md-4 col-lg-3"> 
                <div class="form-group">
                    <label class="labelforms text-muted"><b>Passageiros</b> </label>
                    <input type="text" class="form-control" name="passageiros" value="{{ old('passageiros') ?? $embarcacao->passageiros }}">
                </div>
            </div>
            <div class="col-12 col-md-4 col-lg-3"> 
                <div class="form-group">
                    <label class="labelforms text-muted"><b>Tripulantes</b> </label>
                    <input type="text" class="form-control" name="tripulantes" value="{{ old('tripulantes') ?? $embarcacao->tripulantes }}">
                </div>
            </div>
            <div class="col-12 col-md-4 col-lg-3"> 
                <div class="form-group">
                    <label class="labelforms text-muted"><b>Comprimento</b> </label>
                    <input type="text" class="form-control" name="comprimento" value="{{ old('comprimento') ?? $embarcacao->comprimento }}">
                </div>
            </div>
            <div class="col-12 col-md-4 col-lg-3"> 
                <div class="form-group">
                    <label class="labelforms text-muted"><b>Ano de Construção</b> </label>
                    <input type="text" class="form-control" name="ano_de_construcao" value="{{ old('ano_de_construcao') ?? $embarcacao->ano_de_construcao }}">
                </div>
            </div>
        </div>
        <div class="col-12 mb-1"> 
            <div class="form-group">
                ...
                <input id="tags_1" class="tags" rows="5" name="metatags" value="{{ old('metatags') ?? $embarcacao->metatags }}">
            </div>
        </div>
        <div class="row mb-2">
            <div class="col-12">   
                <label class="labelforms text-muted"><b>Descrição da Embarcação</b></label>
                <x-adminlte-text-editor name="content" v placeholder="Descrição da embarcação..." :config="$config">{{ old('content') ?? $embarcacao->content }}</x-adminlte-text-editor>                                                                                     
            </div>            
        </div>
                ...
                <div class="property_image">
                    ...
                    <div class="property_image_item">
                        ...
                        <div class="property_image_actions">
                            <a href="javascript:void(0)" class="btn btn-xs {{ ($image->cover == true ? 'btn-success' : 'btn-default') }} icon-notext image-set-cover px-2" data-action="{{ route('embarcacoes.imageSetCover', ['image' => $image->id]) }}"><i class="nav-icon fas fa-check"></i> </a>
                            <a href="javascript:void(0)" class="btn btn-danger btn-xs image-remove px-2" data-action="{{ route('embarcacoes.imageRemove', ['image' => $image->id]) }}"><i class="nav-icon fas fa-times"></i> </a>
                        </div>
                    </div>
                    @endforeach
                </div>
